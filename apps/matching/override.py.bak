...
from apps.matching.domain import classify_exception


def validate_override_pair(
    mentor: Participant, mentee: Participant, cohort: Cohort, match_run: MatchRun
) -> Tuple[bool, str]:
    """
...
def get_swap_suggestion(
    mentor: Participant, mentee: Participant, match_run: MatchRun
    ...
    try:
        mentor_existing_match = Match.objects.get(match_run=match_run, mentor=mentor)
        mentor_current_mentee = mentor_existing_match.mentee
    except Match.DoesNotExist:
        mentor_current_mentee = None

    try:
        mentee_existing_match = Match.objects.get(match_run=match_run, mentee=mentee)
    ...
    if (
        mentor_current_mentee
        and mentee_current_mentor
        and mentor_current_mentee != mentee
    ):
...
def create_manual_override(
    ...
    """
    # Validate the pair
    is_valid, error_msg = validate_override_pair(
        mentor, mentee, match_run.cohort, match_run
    )
    if not is_valid:
        return False, error_msg, None

    # Check if this creates an exception
    inputs = prepare_inputs(match_run.cohort)
    ...
    if exception_type and not override_reason.strip():
        return (
            False,
            "Override reason is required when creating an exception match",
            None,
        )
    ...
    try:
        with transaction.atomic():
            ...
            Match.objects.filter(match_run=match_run, mentee=mentee).exclude(
                mentor=mentor
            ).delete()
...
def set_active_match_run(
    cohort: Cohort, match_run: MatchRun, set_by: User
) -> Tuple[bool, str]:
    """
    ...
    try:
        active_run, created = ActiveMatchRun.objects.update_or_create(
            cohort=cohort, defaults={"match_run": match_run, "set_by": set_by}
        )
        if created:
    ...
    try:
        ...
        if participant.role_in_cohort == "MENTOR":
            match = Match.objects.get(
                match_run=active_run.match_run, mentor=participant
            )
        else:  # MENTEE
            match = Match.objects.get(
                match_run=active_run.match_run, mentee=participant
            )
        return match
