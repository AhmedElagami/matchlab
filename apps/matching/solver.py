...
def get_strict_feasible_pairs(
    ...
    for mentor in mentors:
        for mentee in mentees:
            ...
            mutual_acceptable = (
                mentee.id in mentor_accepts.get(mentor.id, set()) and
                mentor.id in mentee_accepts.get(mentee.id, set())
            )
...
def get_pair_scores(
    ...
    for mentor in mentors:
        for mentee in mentees:
            scores[(mentor.id, mentee.id)] = score_lookup.get((mentor.id, mentee.id), 0.0)
...
def solve_strict(
    ...
    """
    logger.info(f"Solving strict matching for {len(mentors)} mentors and {len(mentees)} mentees")
    ...
    logger.info(f"Found {feasible_count} feasible pairs out of {len(mentors) * len(mentees)} total")
    ...
    for i, mentor in enumerate(mentors):
        feasible_mentee_indices = [
            j for j, mentee in enumerate(mentees) 
            if feasible_pairs[(mentor.id, mentee.id)]
        ]
        if feasible_mentee_indices:
            model.AddExactlyOne(x[(i, j)] for j in feasible_mentee_indices)
        # If no feasible mentees, constraint is automatically satisfied (0=1) which makes problem infeasible
    
    # Each mentee matched exactly once
    for j, mentee in enumerate(mentees):
        feasible_mentor_indices = [
            i for i, mentor in enumerate(mentors) 
            if feasible_pairs[(mentor.id, mentee.id)]
        ]
        if feasible_mentor_indices:
            model.AddExactlyOne(x[(i, j)] for i in feasible_mentor_indices)
        # If no feasible mentors, constraint is automatically satisfied (0=1) which makes problem infeasible
    
    # Objective: maximize total score
    if x:  # Only if we have variables
        model.Maximize(sum(x[(i, j)] * int(scores[(mentor.id, mentee.id)] * SCORE_SCALE) 
    ...
    if status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
        ...
        logger.info(f"Found solution with {len(matches)} matches, total score: {total_score}")
        return True, result
        ...
        zero_mentor_options = []
        for i, mentor in enumerate(mentors):
            feasible_count = sum(1 for j, mentee in enumerate(mentees) 
                               if feasible_pairs[(mentor.id, mentee.id)])
            if feasible_count == 0:
        ...
        zero_mentee_options = []
        for j, mentee in enumerate(mentees):
            feasible_count = sum(1 for i, mentor in enumerate(mentors) 
                               if feasible_pairs[(mentor.id, mentee.id)])
            if feasible_count == 0:
        ...
        if matched_mentee:
            ...
            if best_alt_mentee and (matched_score - best_alt_score) <= gap_threshold:
                ...
                for amb in ambiguities:
                    if (amb['participant'].id == matched_mentee.id and 
                        amb['matched_with'].id == mentor.id):
