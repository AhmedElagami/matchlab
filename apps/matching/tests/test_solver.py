...
from apps.matching.domain import detect_ambiguity
from apps.matching.service import run_matching
from apps.matching.export import export_match_run_csv
...
class SolverTest(TestCase):
    ...
    def test_solve_strict_success(self):
        ...
        match_pairs = [(m["mentor_id"], m["mentee_id"]) for m in result.matches]
        expected_pairs = [
            (self.mentor1.id, self.mentee1.id),
            (self.mentor2.id, self.mentee2.id),
        ]
    ...
    def test_detect_ambiguity_no_ambiguity(self):
        ...
        inputs = PreparedInputs(
            ...
            mentee_ids=[self.mentee1.id, self.mentee2.id],
            same_org={
                (self.mentor1.id, self.mentee1.id): False,
                (self.mentor1.id, self.mentee2.id): False,
                (self.mentor2.id, self.mentee1.id): False,
                (self.mentor2.id, self.mentee2.id): False,
            },
            participant_orgs={
                self.mentor1.id: "OrgA",
                self.mentor2.id: "OrgB",
                self.mentee1.id: "OrgB",
                self.mentee2.id: "OrgA",
            },
            acceptability={
                (self.mentor1.id, self.mentee1.id): "MUTUAL",
                (self.mentor1.id, self.mentee2.id): "MUTUAL",
                (self.mentor2.id, self.mentee1.id): "MUTUAL",
                (self.mentor2.id, self.mentee2.id): "MUTUAL",
            },
            score={
                (self.mentor1.id, self.mentee1.id): 90000,
                (self.mentor1.id, self.mentee2.id): 50000,
                (self.mentor2.id, self.mentee2.id): 85000,
                (self.mentor2.id, self.mentee1.id): 40000,
            },
            config={"ambiguity_gap_threshold": 10.0},
        )

        matches = [
            {"mentor_id": self.mentor1.id, "mentee_id": self.mentee1.id, "score": 90.0},
            {"mentor_id": self.mentor2.id, "mentee_id": self.mentee2.id, "score": 85.0},
        ]
    ...
    def test_detect_ambiguity_with_ambiguity(self):
        ...
        inputs = PreparedInputs(
            ...
            score={
                (self.mentor1.id, self.mentee1.id): 90000,
                (self.mentor1.id, self.mentee2.id): 88000,
                (self.mentor2.id, self.mentee2.id): 85000,
                (self.mentor2.id, self.mentee1.id): 82000,
            },
            config={"ambiguity_gap_threshold": 5000.0},
        )
...
class ServiceTest(TestCase):
    ...
    def test_run_matching_strict_success(self):
        """Test that strict run creates MatchRun and Matches."""
