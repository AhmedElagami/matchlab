...
- Production defaults use `Dockerfile` and `docker-compose.yml`.
- Development defaults use `Dockerfile.dev` and `docker-compose.dev.yml`.
- Pick one path and stick with it to avoid mixing dependencies or settings.
