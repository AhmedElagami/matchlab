...
for obj in users:
    ...
    user_map[obj['pk']] = u.id
    print(f"✓ User: {u.username} (ID: {u.id})")

for obj in participants:
    new_user_id = user_map[obj['fields']['user']]
    new_cohort_id = cohort_map[obj['fields']['cohort']]
    p = Participant.objects.create(user_id=new_user_id, cohort_id=new_cohort_id, role_in_cohort=obj['fields']['role_in_cohort'], display_name=obj['fields'].get('display_name', ''), organization=obj['fields'].get('organization', ''), is_submitted=obj['fields'].get('is_submitted', False), created_at=obj['fields']['created_at'], updated_at=obj['fields']['updated_at'])
