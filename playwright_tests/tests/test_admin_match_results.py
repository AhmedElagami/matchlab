...
def test_admin_match_results_page_displays_correctly(page, live_server, admin_user, test_cohort):
    ...
    # Login as admin
    page.fill("[data-testid='login-email']", "admin@test.com")
    page.fill("[data-testid='login-password']", "testpass123")
    page.click("[data-testid='login-submit']")
    
    # Navigate to match results page
    results_url = reverse("admin_views:match_results", kwargs={"match_run_id": match_run.id})
...
def test_admin_match_results_filters_work(page, live_server, admin_user, test_cohort):
    ...
    mentor1 = Participant.objects.create(
        user=User.objects.create_user(username="m1", email="m1@test.com", password="testpass123"),
        cohort=test_cohort,
    ...
    mentee1 = Participant.objects.create(
        user=User.objects.create_user(username="t1", email="t1@test.com", password="testpass123"),
        cohort=test_cohort,
    ...
    mentee2 = Participant.objects.create(
        user=User.objects.create_user(username="t2", email="t2@test.com", password="testpass123"),
        cohort=test_cohort,
